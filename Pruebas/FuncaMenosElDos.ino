...
const float engrMotor[NUM_MOT] = {5, 5.18, 5.18, 5, 3.4716, 3.4716};
...
if (Serial.available()) {
  ...
  // Esperar a que lleguen nuevos datos
    ...
    float velMax[NUM_MOT] = {360, 360, 180, 50, 180, 180};
    ...
    for (int i = 0; i < NUM_MOT && !salirFor; i++) {
      ...
      if (posMot[i] == 3000) {
        ...
        garra.write(60); // Mover la garra a 90 grados
        Serial.println("Abrir la garra");
        salirFor = true;
      } else if (posMot[i] == 3001){
        garra.write(20); // Cerrar la garra
